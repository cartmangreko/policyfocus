...
NODES_COMMENT = [
    ...
    "NEITHER FIGURE IS CORRECTED, and the sum carries `bound: \"lower\"` because it can",
    "only ever be a floor: this sweep sees the orders the OEM PUBLISHED with a",
    "counterparty or a figure, and an order book contains orders nobody announced. The",
    "gap is not a contradiction and must not be read as one.",
    "",
...
EDGES_COMMENT = [
    ...
    "",
    "`verdict` IS NULL ON EVERY EDGE IN THIS FILE AND IS WRITTEN BY GEORGE, BY NOBODY",
    "else and by no re-run. A ruling that settles a whole class is recorded as DATA --",
    "the 98 frameworks were ruled en bloc and that ruling is their `firmness` -- and",
    "their verdicts are null like everything else's. A build that filled in a verdict",
    "would be a build overwriting the reader. DECISION D-19.",
    "",
