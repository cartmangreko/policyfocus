#!/usr/bin/env python3
"""The declared reader for the CROSS-SECTOR funder pass, and its cache index.

    python3 sources/funder_search.py URL [URL ...]

Every fetch brief 11's funder pass makes goes through here: the URL, the day, the
byte count and the SHA-256 land in sources/cache/funders/index.json, and the body
lands beside it under its hash.

SAME READER AS sources/hydrogen_search.py, ccs_search.py, battery_search.py AND
steel_search.py, POINTED AT A DIFFERENT CACHE. It is a copy rather than an import
on the standing reason: each pass is snapshotted separately and a shared module
would let a change made for one silently rewrite another's record of what it did.

WHY A CACHE OF ITS OWN RATHER THAN THE HYDROGEN ONE. Brief 12's funder pass read
hydrogen programmes and wrote them into the hydrogen cache, which was right while
the population was one sector. This pass reads the Innovation Fund's general
calls and the battery IPCEIs, whose awards land on cement, steel, battery and
storage rows. A FUNDER IS NOT A SECTOR, and filing the Commission's award list
under `hydrogen` would say it was.

A NON-ASCII URL IS STILL A URL and this reader percent-encodes before sending;
see sources/steel_docket.md, D-S1, for what the alternative cost.
...
_HEAD = [
    "THE CACHE INDEX FOR THE CROSS-SECTOR FUNDER PASS, brief 11. One entry per fetch: the URL, the day,",
...
def text_of(body: bytes, ctype: str) -> str:
    """THIS READER PARSES PDFs AND ITS SIBLINGS DO NOT, and that is deliberate.

    The funder pass reads the Commission's own documents, and the Commission
    publishes its awards as PDFs: the Innovation Fund project fiche carries the
    coordinator and the grant amount, and the state-aid decision carries the
    participants. Returning "" for those, as the sector readers do, records a
    refusal the publisher never gave — the same mistake in a different coat as the
    diacritic bug of D-S1, where a record said nothing had come back from a request
    that was never sent.

    IT DEGRADES AUDIBLY. Without PyMuPDF the text is empty and the caller is told
    so through the outcome column, which then reads "200, empty body" — wrong, but
    wrong in the direction that makes somebody look. THIS READER IS NEVER IN THE
    BUILD CHAIN, so the package is in sources/requirements.txt and not in
    requirements-gates.txt; see scope.md, "A build-time gate reads tracked files
    only".
    """
    if "pdf" in (ctype or "") or body[:4] == b"%PDF":
        try:
